...
Per allele (folds = c000..c004 used as 5 folds):
  ...
  TRUE  (nested 5-fold CV):
      for each OUTER (eval) fold t:
          ...
          predict outer fold t with the ENSEMBLE (mean) of the 4 inner
              models trained with HP*
...
def true_cv(folds, grid, seed):
    ...
    return smm.pcc(yt, yp), smm.mse(yt, yp), selected
...
def run_allele_folds(allele, folds, grid, seed):
    ...
    true_pcc, true_mse, selected = true_cv(folds, grid, seed)
...
def main():
    ...
    print(f"\nWrote {args.out_prefix}_fake_grid.csv and {args.out_prefix}_summary.csv",
          file=sys.stderr)
